...
Upon receiving a Power server, I make sure to do these things to get it up and running (Assuming I have an HMC set up with VIOS images uploaded to it):
  ...
  - Run `vios` role
    > **Note:** The underlying `ibm.power_hmc.vios` module that the `vios` role uses assumes you are using Fibre Channel-backed storage and doesn't
  ...
  - Link aggregate needed interfaces and create virtual networks on top of them
    > **Note:** As a bonus, if you include the same physical interface you used for the VIOS install into a link aggregation bundle, VIOS traffic
  ...
  - Create needed logical partitions, then:
    ...
    > **Note:** Likewise, this step needs to be done manually. The underlying `ibm.power_hmc.powervm_lpar_instance` module that the `lpar`
